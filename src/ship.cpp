...
ship::ship(game &gm_, const xml_elem &parent)
    ...
    if (parent.has_child("ai")) {
        ...
        if (aitype == "dumb")
            myai.reset(new ai(this, ai::dumb));
        else if (aitype == "escort")
            myai.reset(new ai(this, ai::escort));
        else if (aitype == "none")
            myai.reset();
        else
    ...
    for (unsigned i = 0; i < noise::NR_OF_FREQUENCY_BANDS; ++i) {
        noise_sign.band_data[i].basic_noise_level = noise_signature::typical_noise_signature[unsigned(myclass)][i];
...
void ship::compute_force_and_torque(vector3 &F, vector3 &T) const {
    ...
    */
    ...
    const float voxel_vol = voxel_size.x * voxel_size.y * voxel_size.z * volume_scale;
    const double voxel_vol_force = voxel_vol * GRAVITY * 1000.0; // 1000kg per cubic meter
    const matrix4f transmat = orientation.rotmat4() * mymodel->get_base_mesh_transformation() * matrix4f::diagonal(voxel_size);
    ...
    */
    ...
    vector3 local_torque = tvr2.coeff_mul(area).coeff_mul(L.coeff_mul(L).coeff_mul(L)) * (drag_coefficient * water_density * 0.125);
    if (w.x > 0.0)
