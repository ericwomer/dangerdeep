...
class non_uniform_bsplinet {
  ...
  public:
    non_uniform_bsplinet(unsigned n_, const std::vector<T> &d, const std::vector<double> &tvec_)
        ...
        if (n >= d.size())
            throw std::runtime_error("bspline: n too large");
        if (d.size() < 2)
            throw std::runtime_error("bspline: d has too few elements");
        if (tvec_.size() != m + n + 2)
            throw std::runtime_error("bspline: tvec has illegal size");
        ...
        */
        ...
        for (unsigned r = 1; r <= n; ++r) {
            for (unsigned i = l - n; i <= l - r; ++i) {
                ...
                if (!isfinite(tv))
                    throw std::runtime_error("bspline: invalid number generated");
                deBoor_at(r, i + n - l) = deBoor_at(r - 1, i + n - l) * (1 - tv) + deBoor_at(r - 1, i + 1 + n - l) * tv;
            }
        }
  ...
  public:
    bsplinet(unsigned n_, const std::vector<T> &d)
        ...
        for (; k <= this->n; ++k)
            this->tvec[k] = 0;
        for (; k <= this->m; ++k)
            this->tvec[k] = double(k - this->n) / double(this->m - this->n + 1);
        for (; k <= this->m + this->n + 1; ++k)
            this->tvec[k] = 1;
    }
};
...
class bspline2dt {
  ...
  public:
    ...
    bspline2dt(unsigned n_, const std::vector<T> &d) : n(n_), cp(d) {
        ...
        if (ds * ds != d.size())
            throw std::runtime_error("bspline2d: d not quadratic");
        if (n >= ds)
            throw std::runtime_error("bspline2d: n too large");
        if (ds < 2)
            throw std::runtime_error("bspline2d: d has too few elements");
        m = ds - 1;
    ...
    T value(double s, double t) const {
        if (s < 0.0 || s > 1.0)
            throw std::runtime_error("bspline2d: invalid s");
        if (t < 0.0 || t > 1.0)
            throw std::runtime_error("bspline2d: invalid t");
        ...
        for (unsigned r = 1; r <= n; ++r) {
            for (unsigned i = l - n; i <= l - r; ++i) {
                ...
                if (!isfinite(tv))
                    throw std::runtime_error("bspline2d: invalid number generated");
                for (unsigned j = 0; j <= n; ++j) {
                    deBoor_at(j, r, i + n - l) = deBoor_at(j, r - 1, i + n - l) * (1 - tv) + deBoor_at(j, r - 1, i + 1 + n - l) * tv;
                }
            }
        }
        ...
        for (unsigned r = 1; r <= n; ++r) {
            for (unsigned i = l2 - n; i <= l2 - r; ++i) {
                ...
                if (!isfinite(tv))
                    throw std::runtime_error("bspline2d: invalid number generated");
                deBoor_at(0, r, i + n - l2) = deBoor_at(0, r - 1, i + n - l2) * (1 - tv) + deBoor_at(0, r - 1, i + 1 + n - l2) * tv;
            }
        }
