...
vector<Uint8> particle::make_2d_smoothed_noise_map(unsigned wh) {
    ...
    for (unsigned y = 0; y < wh; ++y) {
        ...
        for (unsigned x = 0; x < wh; ++x) {
            ...
            unsigned r = (unsigned(tmp[y1 * wh + x1]) + unsigned(tmp[y1 * wh + x2]) + unsigned(tmp[y2 * wh + x1]) + unsigned(tmp[y2 * wh + x2])) / 16 + (unsigned(tmp[y * wh + x1]) + unsigned(tmp[y * wh + x2]) + unsigned(tmp[y1 * wh + x]) + unsigned(tmp[y2 * wh + x])) / 8 + unsigned(tmp[y * wh + x]) / 4;
            Uint8 r2 = (Uint8)(r);
            tmp2[y * wh + x] = r2;
            if (r2 > rmax)
                rmax = r2;
...
vector<Uint8> particle::make_2d_perlin_noise(unsigned wh, unsigned highestlevel) {
    ...
    for (unsigned y = 0; y < wh; ++y) {
        for (unsigned x = 0; x < wh; ++x) {
            unsigned r = 0;
            for (unsigned i = 0; i < levels; ++i) {
                r += interpolate_2d_map(lookup_maps[i], 1 << (highestlevel + i), x, y, wh) * 65536 / (1 << (i + 1));
            }
            r /= 65536;
            if (r > 255)
                r = 510 - r;
    ...
    if (life > 0.666) {
        ...
        for (unsigned i = 0; i < flares.size(); ++i) {
            ...
            for (unsigned k = 0; k < fls; ++k) {
                ...
                vector3 p = position - viewpos + dx * (f.velocity.x * lifefac3 * 2.0) + dy * (f.velocity.y * lifefac3 * 2.0);
                ...
                p = position - viewpos + dx * (f.velocity.x * lifefac3 * 2.0) + dy * (f.velocity.y * lifefac3 * 2.0);
                kk += 1.f / fls;
                t = t0 * (1.0 - kk) + life * kk;
                p.z += get_z(t) - position.z;
        ...
        for (unsigned i = 0; i < flares.size(); ++i) {
            const flare &f = flares[i];
            vector3 p = position - viewpos + dx * (f.velocity.x * lifefac2 * 2.0) + dy * (f.velocity.y * lifefac2 * 2.0);
