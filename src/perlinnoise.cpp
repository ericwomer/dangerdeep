...
perlinnoise::perlinnoise(unsigned size, unsigned sizeminfreq, unsigned sizemaxfreq)
    : resultsize(size) {
    if (!is_power2(size))
        throw std::invalid_argument("size is not power of two");
    if (!is_power2(sizeminfreq))
        throw std::invalid_argument("sizeminfreq is not power of two");
    if (!is_power2(sizemaxfreq))
        throw std::invalid_argument("sizemaxfreq is not power of two");
    if (!(sizeminfreq >= 1 && sizeminfreq <= size && sizeminfreq <= sizemaxfreq))
        throw std::invalid_argument("sizeminfreq out of range");
    if (!(sizemaxfreq >= 2 && sizemaxfreq <= size))
        throw std::invalid_argument("sizemaxfreq out of range");
    unsigned nrfunc = 0;
...
perlinnoise::perlinnoise(unsigned levelsize, unsigned sizeminfreq, unsigned levels, bool /*dummy*/) {
    if (!is_power2(levelsize))
        throw std::invalid_argument("levelsize is not power of two");
    if (!is_power2(sizeminfreq))
        throw std::invalid_argument("sizeminfreq is not power of two");
    if (levels < 1)
        throw std::invalid_argument("levels must be >= 1");
