...
class quaterniont {
  public:          // we use a spatial representation
    ...
    static quaterniont<D> rot(const D &angle, const D &x, const D &y, const D &z) {
        D rad = D(angle * M_PI / 360.0);
...
template <class D2>
inline quaterniont<D2> operator*(const D2 &scalar, const quaterniont<D2> &q) { return q * scalar; }
