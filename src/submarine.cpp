...
void submarine::init_fill_torpedo_tubes(const date &d) {
    ...
    for (unsigned i = idx.first; i < idx.second; ++i) {
        unsigned r = rnd(6);
        if (r <= 1)
            torpedoes[i] = stored_torpedo(gm, standard1);
        else if (r <= 2)
            torpedoes[i] = stored_torpedo(gm, standard2);
        else if (r <= 3)
            torpedoes[i] = stored_torpedo(gm, special1);
        else
            torpedoes[i] = stored_torpedo(gm, special2);
    }
...
void submarine::ballast_tank_control_logic(double delta_time) {
    ...
    if (dive_state == dive_state_surfaced || dive_state == dive_state_preparing_for_dive || (!permanent_dive && dive_to > -1.0)) {
        err = 1;
...
bool submarine::launch_torpedo(int tubenr, sea_object *target) {
    if (target == 0)
        return false; // maybe assert this?
    if (target == this)
        return false;
    ...
    if (tubenr < 0) {                                                             // check if target is behind
        angle a = angle(target->get_pos().xy() - get_pos().xy()) - get_heading(); // + trp_addleadangle;
    ...
    if (TDC.solution_valid()) {
        ...
        doc.load();
        auto torp = std::make_unique<torpedo>(gm, doc.first_child(), torpedoes[tubenr].setup);
