...
bivector<T> bivector<T>::shifted(const vector2i &offset) const {
    ...
    bivector_FOREACH_XYZ(result.at((x + offset.x) & (datasize.x - 1), (y + offset.y) & (datasize.y - 1)) = data[z]) return result;
}
...
bivector<T> &bivector<T>::add_tiled(const bivector<T> &other, const T &scal) {
    bivector_FOREACH_XY(at(x, y) += other.at(x & (other.datasize.x - 1), y & (other.datasize.y - 1)) * scal;) return *this;
}

template <class T>
bivector<T> &bivector<T>::add_shifted(const bivector<T> &other, const vector2i &offset) {
    bivector_FOREACH_XY(at(x, y) += other.at((x + offset.x) & (other.datasize.x - 1), (y + offset.y) & (other.datasize.y - 1));) return *this;
}
