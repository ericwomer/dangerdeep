...
void texts::read_category(category ct) {
    ...
    for (unsigned i = 0; i < available_language_codes.size(); ++i) {
        if (!p.next_column())
            p.error("no columns left in texts file");
        string lc = p.get_cell();
        if (lc != available_language_codes[i])
            p.error(string("invalid language code marker found, expected \"") + available_language_codes[i] + "\", got \"" + lc + "\"!");
        if (lc == language_code)
            lcn = i;
    }
...
void texts::set_language(unsigned nr) {
    ...
    if (nr >= available_language_codes.size())
        throw error(string("trying to set illegal language nr, valid 0...") + str(available_language_codes.size()) + ", requested " + str(nr));
...
string texts::get(unsigned no, category ct) {
    ...
    if (no >= tx.size())
        throw error(string("invalid text nummer for texts::get() ") + str(no) + string(", valid 0...") + str(tx.size()) + string(" category=") + str(int(ct)));
    return tx[no];
}
