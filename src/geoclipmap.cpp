...
geoclipmap::level::level(geoclipmap &gcm_, unsigned idx, bool outmost_level)
    ...
    // mostly static...
    vertices.init_data(gcm.resolution_vbo * gcm.resolution_vbo * geoclipmap_fperv * 4 + (outmost ? 8 * geoclipmap_fperv * 4 : 0),
                       0, GL_STATIC_DRAW);
...
void geoclipmap::level::update_region(const geoclipmap::area &upar) {
    if (upar.empty())
        throw error("update area empty?! BUG!"); // continue; // can happen on initial update
...
void geoclipmap::level::display(const frustum &f, bool is_mirror) const {
...
#ifdef DEBUG_INDEX_USAGE
    for (unsigned i = gcm.idxscratchbuf.size() - DEBUG_INDEX_EXTRA; i < gcm.idxscratchbuf.size(); ++i)
    ...
    // render the data
    if (nridx < 4)
        return; // first index is remove always, and we need at least 3 for one triangle
    vertices.bind();
