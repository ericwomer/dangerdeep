  ...
  public:
    ...
    morton_bivector(const long &sz, const T &v = T()) : datasize(sz), data(sz * sz, v) {
        if (log2(sz) != (int)log2(sz))
            throw "morton_bivector::morton_bivector() wrong datasize: size is not power of 2";
        morton_x.resize(sz);
        morton_y.resize(sz);
...
T morton_bivector<T>::get_min() const {
    if (data.empty())
        throw std::invalid_argument("morton_bivector::get_min data empty");
    T m = data[0];
...
T morton_bivector<T>::get_max() const {
    if (data.empty())
        throw std::invalid_argument("morton_bivector::get_max data empty");
    T m = data[0];
...
T morton_bivector<T>::get_min_abs() const {
    if (data.empty())
        throw std::invalid_argument("morton_bivector::get_min_abs data empty");
...
T morton_bivector<T>::get_max_abs() const {
    if (data.empty())
        throw std::invalid_argument("morton_bivector::get_max_abs data empty");
    T m = bivector_abs(data[0]);
...
morton_bivector<T> morton_bivector<T>::sub_area(const vector2i &offset, const long &sz) const {
    if (offset.y + sz > datasize)
        throw std::invalid_argument("morton_bivector::sub_area, offset.y invalid");
    if (offset.x + sz > datasize)
        throw std::invalid_argument("morton_bivector::sub_area, offset.x invalid");
...
morton_bivector<T> morton_bivector<T>::shifted(const vector2i &offset) const {
    ...
    bivector_FOREACH_XYZ(result.at((x + offset.x) & (datasize - 1), (y + offset.y) & (datasize - 1)) = data[z]) return result;
}
...
morton_bivector<U> morton_bivector<T>::convert(const T &minv, const T &maxv) const {
    bivector<U> result(datasize);
    bivector_FOREACH(result.data[z] = U(std::max(minv, std::min(maxv, data[z])))) return result;
}
...
morton_bivector<T> morton_bivector<T>::smooth_upsampled(bool wrap) const {
    ...
    */
    if (datasize < 3)
        throw std::invalid_argument("morton_bivector::smooth_upsampled base size invalid");
    long resultsize = wrap ? datasize * 2 : datasize * 2 - 1;
    morton_bivector<T> result(resultsize);
    // copy values that are kept and interpolate missing values on even rows
    for (int y = 0; y < datasize; ++y) {
...
morton_bivector<T> &morton_bivector<T>::add_tiled(const bivector<T> &other, const T &scal) {
    bivector_FOREACH_XY(at(x, y) += other.at(x & (other.datasize - 1), y & (other.datasize - 1)) * scal;) return *this;
}

template <class T>
morton_bivector<T> &morton_bivector<T>::add_shifted(const bivector<T> &other, const vector2i &offset) {
    bivector_FOREACH_XY(at(x, y) += other.at((x + offset.x) & (other.datasize - 1), (y + offset.y) & (other.datasize - 1));) return *this;
}
