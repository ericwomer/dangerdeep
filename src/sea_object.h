...
class sea_object {
  public:
    ...
    enum sensor_system { lookout_system,
                         radar_system,
    ...
    struct damage_data_scheme {
        ...
        damage_data_scheme(const vector3f &a, const vector3f &b, float w, unsigned t, bool s = false, bool r = true) : p1(a), p2(b), weakness(w), repairtime(t), surfaced(s), repairable(r) {}
    };
  ...
  public:
    ...
    virtual void set_target(sea_object *s) {
        if (s && s->is_alive())
            target = s;
    }
