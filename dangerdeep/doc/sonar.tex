...
device of World War 2, the ``GHG'' (Gruppenhorchgeraet).
...
$(2.1*10^{-10} * (T-38)^2 + 1.3*10^{-7}) * f^2$ (unit: $dB/m$), where
$T$ is temperature in centigrade. We thus get: $I_{absorb} =
\frac{I_0}{m_f * R}$.
...
3kHz, 3\ldots 6kHz, 6\ldots 7kHz).  Background noise should be decreases
...
(clamped) at one, thus if it is less than one, set it to one. The
resulting value is fed to the sonarman simulator:

$$L_{result} = \lfloor \phi^{-1}(\max(1, \frac{I_{targets} - I_{backgr}}{\phi(G_{s_p})})) 
\rfloor$$
