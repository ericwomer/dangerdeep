...
void main()
{
	...
	for(int i=0; i<regions.length(); i++) {
		...
		blended_color  += weight * texture2D(terrain_texture, (frac(gl_TexCoord[0].xy)*tex_coord_factor)+vec2(regions[i].z, regions[i].w)).xyz;
	}
	vec3 slope_color = texture2D(terrain_texture, (frac(gl_TexCoord[0].xy)*tex_coord_factor)+slope_offset).xyz;

	blended_color = mix(blended_color, slope_color, slope);
	

	vec3 final_color = blended_color * (max(dot(L, N), 0.0) * (1.0 - ambient) + ambient);
