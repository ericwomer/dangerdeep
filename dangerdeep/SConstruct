...
if sys.platform == 'win32':
	...
	if (debug >= 4):
		ccflags += ' -std=c++03 -g -pg -O3' # profiling
	elif (debug == 3):
		ccflags += ' -std=c++03 -g3 -ggdb3 -O0'
		env.Append(CPPDEFINES = ['DEBUG'])
	elif (debug == 2):
		ccflags += ' -std=c++03 -g -O1'
		env.Append(CPPDEFINES = ['DEBUG'])
	elif (debug == 1):
		# same as debug==0 but with DEBUG defined to make logging work (and probably other stuff)
		ccflags += ' -std=c++03 -g -O2'
		env.Append(CPPDEFINES = ['DEBUG'])
	elif (debug == -1):
		ccflags += ' -std=c++03 -g -O3 -mfpmath=sse -mmmx -msse -m3dnow'
	elif (debug == -2):	# special g++4.0+ auto vectorization
		ccflags += ' -std=c++03 -g -O3 -mfpmath=sse -mmmx -msse -m3dnow -ftree-vectorize'
	elif (debug == -3):	# special g++4.0+ auto vectorization
		ccflags += ' -std=c++03 -g -O3 -mfpmath=sse -mmmx -msse -msse2 -m3dnow -ftree-vectorize'
	elif (debug == -4):
		ccflags += ' -std=c++03 -g -Ofast -march=native -msse -msse2 -mavx -mavx2 -mbmi -mbmi2 -ftree-vectorize'
	else:
		if (useicc >= 1):
			env.Replace(CC = "icc")
