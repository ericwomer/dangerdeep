...
template <class T>
rastered_map<T>::rastered_map(const std::string& header_file, const std::string& data_dir, unsigned _num_levels) 
	: num_levels(_num_levels), pn(64, 2, 16), pn2(64, 4, num_levels - 4, true)
{
	...
	m_tile_cache = std::auto_ptr<tile_cache<T> >(new tile_cache<T>(data_dir, bounds.x, bounds.y, 512, 4, 0, min_height-1));

 }

template <class T>
void rastered_map<T>::compute_heights(int detail, const vector2i& coord_bl, const vector2i& coord_sz,
...
template <class T>
void rastered_map<T>::compute_colors(int detail, const vector2i& coord_bl, const vector2i& coord_sz, Uint8* dest)
{
    for (int y = 0; y < coord_sz.y; ++y) {
        for (int x = 0; x < coord_sz.x; ++x) {
            vector2i coord = coord_bl + vector2i(x, y);
