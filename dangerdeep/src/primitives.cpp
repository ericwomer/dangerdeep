...
primitives::primitives(int type_,
		       bool with_colors, bool with_texcoords,
		       bool with_normals, unsigned size)
	: type(type_)
{
	vertices.resize(size);
	if (with_colors) colors.resize(size);
	if (with_texcoords) texcoords.resize(size);
	if (with_normals) normals.resize(size);
}
...
primitives primitives::textured_quad(const vector2f& xy0,
				     const vector2f& xy1,
				     const vector2f& texc0,
				     const vector2f& texc1)
{
	primitives result(GL_QUADS, false, true, false, 4);
