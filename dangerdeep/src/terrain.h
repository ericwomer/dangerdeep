...
protected:
	...
	vector2l bounds;
	vector2l origin;
	float noise_h, noise_lac, noise_off, noise_scale;
	...
	bivector<float> patch;
        if (detail < (num_levels - 1)) {
			...
			for (int y = 0; y < coord_sz.y; ++y) {
				for (int x = 0; x < coord_sz.x; ++x) {
					vector2i coord = coord_bl + vector2i(x, y);
					patch.at(x,y) += fractal->get_value(vector3f(coord.x<<(detail+1), coord.y<<(detail+1), patch.at(x,y)),num_levels-detail)*noise_scale;
				}
			}
